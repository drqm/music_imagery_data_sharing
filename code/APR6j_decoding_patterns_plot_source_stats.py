''' Script to plot statistical analyses of source localized decoding patterns '''
...
for p in periods:
    if (tmin == 0) & (tmax == 4):
        ...
        csfname = '{}patterns_source_stats_{}_peaks_tmin_{}_tmax_{}.p'.format(stats_dir, p, tmin,tmax)
...
MA = {}
clust_report = {'period': [], 'con#dition': [], 'cluster No': [],
                 'x': [], 'y': [], 'z': [],'p-value': [],
...
for p in stats:
    ...
    for c in conds:   
        ...
        cname = '{} {}'.format(p,c)
        print(cname)
        cfig, caxis = plt.subplots(nrows=2,ncols=2,subplot_kw={'projection': '3d'},figsize=(4,4))
        ...
        for hix,h in enumerate(hemi):
            for vix, vi in enumerate(views):
                ...
                plotting.plot_surf_stat_map(surfs2[hix],surf, hemi=h,axes=caxis[vix,hix],
                                                ...
                                                colorbar=False, bg_map=bgs[hix],darkness=1)#,#,
        ...
        plt.show()
        cfig.savefig(figures_dir + 'patterns_sources_' + cname + f'_all_clust_surface_tmin_{tmin}_tmax_{tmax}.pdf',dpi=300)
        if export_data:
            print('exporting data')
            surf_df = pd.DataFrame({ck: np.concatenate(surf_df[ck]) for ck in surf_df})
            surf_df = surf_df.round(3)
            surf_df.to_csv(stats_dir + 'plot_data_patterns_sources_' + cname + f'_all_clust_surface_tmin_{tmin}_tmax_{tmax}.csv', index=False)
